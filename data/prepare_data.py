...
def resize_multiple(img, sizes=(16, 128), resample=cv2.INTER_CUBIC, lmdb_save=False):
...
def resize_worker(img_file, sizes, resample, lmdb_save=False):
    ...
    img = cv2.imread(img_file, cv2.IMREAD_ANYDEPTH | cv2.IMREAD_UNCHANGED)
    #img = img[:, :, np.newaxis]
    out = resize_multiple(
        img, sizes=sizes, resample=resample, lmdb_save=False)

    return os.path.splitext(os.path.basename(img_file))[0], out
...
def prepare_process_worker(wctx, file_subset):
    for file in file_subset:
        ...
        if not wctx.lmdb_save:
          cv2.imwrite('{}/lr_{}/{}.tif'.format(wctx.out_path, wctx.sizes[0], i.zfill(5)),lr_img)
          cv2.imwrite('{}/hr_{}/{}.tif'.format(wctx.out_path, wctx.sizes[1], i.zfill(5)),hr_img)
          cv2.imwrite('{}/sr_{}_{}/{}.tif'.format(wctx.out_path, wctx.sizes[0], wctx.sizes[1], i.zfill(5)),sr_img)
        else:
...
def prepare(img_path, out_path, n_worker, sizes=(16, 128), resample=cv2.INTER_CUBIC, lmdb_save=False):
    ...
    if n_worker > 1:
        ...
        for file in tqdm(files):
            ...
            if not lmdb_save:
              cv2.imread('{}/lr_{}/{}.tif'.format(out_path, sizes[0], i.zfill(5)),lr_img, cv2.IMREAD_ANYDEPTH )
              cv2.imread('{}/hr_{}/{}.tif'.format(out_path, sizes[1], i.zfill(5)),hr_img, cv2.IMREAD_ANYDEPTH )
              cv2.imread( '{}/sr_{}_{}/{}.tif'.format(out_path, sizes[0], sizes[1], i.zfill(5)),sr_img, cv2.IMREAD_ANYDEPTH )
...
if __name__ == '__main__':
    ...
    resample_map = {'bicubic': cv2.INTER_CUBIC}
